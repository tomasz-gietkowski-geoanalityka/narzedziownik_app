...
# ➕ DODANE: dopisujemy pole "nazwa"
OUT_FIELDS = ("profil_podstawowy", "profil_dodatkowy", "nazwa")
...
def collect_titles_from_gml(gml_path: str):
    # map: strefa_gml_id -> {"podstawowy": set(), "dodatkowy": set(), "nazwa": str|None}
    titles = {}
    ...
    for event, elem in context:
        ...
        if event == 'start':
            if ln == 'StrefaPlanistyczna':
                ...
                collecting = True
                if cur_id:
                    titles.setdefault(cur_id, {"podstawowy": set(), "dodatkowy": set(), "nazwa": None})
            ...
            # ➕ DODANE: odczyt app:nazwa → xlink:title
            elif collecting and ln == 'nazwa':
                t = get_attr_local(elem.attrib, 'title')  # xlink:title
                if t and cur_id and titles[cur_id].get("nazwa") is None:
                    titles[cur_id]["nazwa"] = t
        elif event == 'end':
            if ln == 'StrefaPlanistyczna':
                collecting = False
                cur_id = None
...
def run(iface, plugin_dir: str):
    ...
    for f in strefa_src.getFeatures():
        ...
        if sid and sid in titles_by_id:
            ...
            # ➕ DODANE: pobranie nazwy (string lub None)
            nz = titles_by_id[sid].get("nazwa")
